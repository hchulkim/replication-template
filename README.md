...
| "`ssggregate` package" | [`ssaggregate` R package](https://github.com/kylebutts/ssaggregate) | No physical files | TRUE (provided as an installed package from the GitHub repository.) | For converting "location-level" variables in a shift-share IV dataset to a dataset of exposure-weighted “industry-level” aggregates following Borusyak, Hull, and Jaravel (2022), we use a public R package named `ssaggregate` provided at https://github.com/kylebutts/ssaggregate. In order to preserve the R package dependency, we also compress the GitHub repository in `data/ssaggregate-main.zip`. |
